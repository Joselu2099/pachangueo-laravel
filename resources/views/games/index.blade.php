<?php setlocale(LC_ALL, 'es_ES.UTF-8'); ?>
@extends('layouts.app')
@section('title', $viewData['title'])
@section('stylesheet')
    <link href="{{ asset('/css/games.css') }}" rel="stylesheet"/>
@endsection
@section('content')
    <div class="subheader">
        <h1>Mis Pachangas</h1>
        @guest
            <h4>Tienes que estar registrado o entrar con tu usuario para acceder a esa sección.</h4>
    </div>
        @else
        <a href="{{ route('games.crud.create') }}" class="btn btn-primary">Crear Pachanga</a>
    </div>
        <div class="container games">
            <div class="row">
                @foreach($viewData["games"] as $game)
                    <div class="card game-card" style="width: 18rem;">
                        <div class="card-header">
                            <h5 class="card-title">{{ $game->getLocation() }}</h5>
                            <h6 class="card-subtitle">{{ strftime("%d de %B", strtotime($game->getDate())) }}</h6>
                        </div>
                        <div class="card-body">
                            <h6 class="card-sport">Deporte: <span>{{ $game->getSport() }}</span></h6>
                            <p class="card-text">{{ substr($game->getDescription(), 0, 150) }}</p>
                            <div class="card-group-buttons">
                                @if(\Illuminate\Support\Facades\Auth::id()==$game->getCreator())
                                    <a href="{{ route('games.crud.edit', $game->getId()) }}" class="btn btn-warning">
                                        <img class="app-icon" src="{{ asset('/images/edit.png') }}">
                                    </a>
                                    <a href="#" class="btn btn-danger"
                                       onclick="if(confirm('¿Esta seguro de que desea borrar esta pachanga?')) { document.getElementById('delete-form-{{ $game->getId() }}').submit(); }">
                                        <img class="app-icon" src="{{ asset('/images/delete.png') }}">
                                    </a>
                                    <form id="delete-form-{{ $game->getId() }}"
                                          action="{{ route('games.crud.delete', $game->getId()) }}" method="post"
                                          style="display: none;">
                                        @method('delete')
                                        @csrf
                                    </form>
                                @endif
                                <a href="{{ route('games.show', $game->getId()) }}" class="btn btn-primary">
                                    <img class="app-icon" src="{{ asset('/images/ver_mas.png') }}">
                                </a>
                            </div>
                        </div>
                    </div>
                @endforeach
            </div>
        </div>
    @endguest
@endsection
